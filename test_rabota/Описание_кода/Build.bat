rem Записываем текущее время в лог фаил
@echo %TIME:~0,8% - Обновляем дату сборки>>%LOG_FILE%         

rem Если условие выполняется делаем переход на NO_KOMPIL
if "%3"=="HAND_AVTO" goto NO_KOMPIL

@echo *** Обновляем дату сборки ***
rem Снимаем аттрибут файлу чтение
...
rem Запуск процесса с параметрами
...
rem Проверяем существование файлов, если нет файла переходим на указанную метку COMPILATION_FAILED
...
rem Сравниваем параметры из командной строки
if "%1"=="NIGHT" (
...
rem Удаление без запроса
del /Q "%WORK_DIR%\sbis\Модули\sbis\muzzle-tests.dll"
del /Q "%WORK_DIR%\sbis\Модули\sbis\sbis-tests.dll"     

rem Метка(if "%3"=="HAND_AVTO" goto NO_KOMPIL)
:NO_KOMPIL

rem Записываем текущую дату в фаил
...
rem Метка , если какой то фаил не был найден
:COMPILATION_FAILED
@echo.
@echo ****************************************************
@echo Ошибка при сборке одного из проектов!!!
@echo ****************************************************
rem Переход на метку
@goto ERRORS
